...
import {setSelectedHospitalId, setSelectedSessionCard, searchSessions} from "@/store/booking/bookingSlice";
...
export const StepTypeAndDate: React.FC<StepTypeAndDateProps> = ({
  ...
  // State for sessions from backend
  const [allSessions, setAllSessions] = useState<SessionCard[]>([]);

  // Fetch sessions from backend on mount or when doctor/filter changes
  useEffect(() => {
    if (doctorId) {
      dispatch(
        searchSessions({
          doctorId,
          location: filterHospitalId || undefined,
        })
      )
        .unwrap()
  ...
  const uniqueLocations = useMemo(() => {
    ...
    allSessions.forEach(session => {
      if (session.hospitalLocation && session.appointmentType === "in-person") {
  ...
  const groupSessionsByDate = (sessions: SessionCard[]): { date: string; dateFormatted: string; sessions: SessionCard[] }[] => {
    const groups: { [key: string]: { date: string; dateFormatted: string; sessions: SessionCard[] } } = {};
  ...
  const { groupedSessions, totalSessions, totalPages } = useMemo(() => {
    ...
    let sessions = allSessions.filter((s) => s.appointmentType === appointmentType);
  
    // Then filter by location (only for in-person)
    if (appointmentType === "in-person" && filterHospitalId) {
      sessions = sessions.filter((s) => s.hospitalLocation === filterHospitalId);
    }
  ...
  const paginatedGroups = useMemo(() => {
    ...
    let sessionCount = 0;
    const result: { date: string; dateFormatted: string; sessions: SessionCard[] }[] = [];
    ...
    for (const group of groupedSessions) {
      ...
      const groupEndIdx = Math.min(group.sessions.length, groupStartIdx + (endIdx - sessionCount));
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          >
            <div className="flex items-start gap-3">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${
                appointmentType === "in-person" ? "bg-green-100" : "bg-gray-100"
              }`}>
                <svg
                  className={`w-6 h-6 ${
                    appointmentType === "in-person" ? "text-green-600" : "text-gray-600"
                  }`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
              ...
              <div className="flex-1">
                <h4 className={`font-semibold text-base mb-0.5 ${
                  appointmentType === "in-person" ? "text-green-800" : "text-gray-900"
                }`}>
                  In-Person Visit
                </h4>
                <p className={`text-sm ${
                  appointmentType === "in-person" ? "text-green-700" : "text-gray-600"
                }`}>
          ...
          >
            <div className="flex items-start gap-3">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${
                appointmentType === "video-consultation" ? "bg-green-100" : "bg-gray-100"
              }`}>
                <svg
                  className={`w-6 h-6 ${
                    appointmentType === "video-consultation" ? "text-green-600" : "text-gray-600"
                  }`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
              ...
              <div className="flex-1">
                <h4 className={`font-semibold text-base mb-1 ${
                  appointmentType === "video-consultation" ? "text-green-800" : "text-gray-900"
                }`}>
                  Video Consultation
                </h4>
                <p className={`text-sm ${
                  appointmentType === "video-consultation" ? "text-green-700" : "text-gray-600"
                }`}>
        ...
        {isLoadingSessions ? (
          ...
          <div className="text-center py-12 bg-gray-50 rounded-xl border-2 border-dashed border-gray-300">
            ...
            <p className="text-sm text-gray-500 mt-1">
              {filterHospitalId ? 'Try selecting a different hospital or view all hospitals.' : 'Please check back later.'}
            </p>
          </div>
        ) : (
          <>
